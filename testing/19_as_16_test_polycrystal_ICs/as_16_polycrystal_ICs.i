...
[Kernels]
  ...
  active = 'PolycrystalKernel TensorMechanics bndsDiff time_c w_dot coupled_res coupled_parsed'
  [./PolycrystalKernel]
...
[AuxKernels]
  ...
  [./failed]
    type = ParsedAux
    variable = failed
    function = if(sigma11_aux>0.5,1,0)
    args = sigma11_aux
    execute_on = timestep_end
  [../]
[]
...
[BCs]
  ...
  active = 'Pressure fixed_x fixed_y left'
  [./left]
    type = DirichletBC
    variable = c
    boundary = left
...
[Materials]
  active = 'stress CuGrGr strain conc_elas diffusionMat constants local_energy'
  [./CuGrGr]
    # Material properties
  ...
  [./diffusionMat]
    type = ParsedMaterial
    function = if(bnds>0.99,grainDiff,boundDiff)
    f_name = D
    args = bnds
  ...
  [./constants]
    ...
    prop_values = '8.125e-16*6.24150934e+18*1e+09^2*1e-27 2.2841e-26*1e+09^2/6.24150934e+18/1e-27'
  [../]
  [./local_energy]
    type = DerivativeParsedMaterial
    ...
    function = eV_J*d*(A*c2+B*(1-c2)+C*c2*log(c2)+D*(1-c2)*log(1-c2)+E*c2*(1-c2)+F*c2*(1-c2)*(2*c2-1)+G*c2*(1-c2)*(2*c2-1)^2)
    f_name = f_loc
    args = c2
